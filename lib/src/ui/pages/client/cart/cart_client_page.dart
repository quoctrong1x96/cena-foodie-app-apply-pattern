...
class CartShowList {
  Store store;
  List<Cart> listCart;
  CartShowList({required this.store, required this.listCart});
  double total() {
    double result = 0;
    if (listCart.isNotEmpty) {
      for (var product in listCart) {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<CartBloc, CartState>(builder: (context, state) {
      ...
      if (listStore != null) {
        ...
        for (var storeItem in listStore!) {
          ...
          if (listProduct.isNotEmpty) {
            listCarts
                .add(CartShowList(store: storeItem, listCart: listProduct));
          }
        }
      } else {
        Iterable<Cart> productCarts = [];
      ...
      for (var item in listCarts) {
        for (var productItem in item.listCart) {
                ...
                child: BlocBuilder<CartBloc, CartState>(
                    builder: (context, state) => (state.quantityCart -
                        ...
                        ? ListView.builder(
                            ...
                            itemCount: listCarts[0].listCart.length,
                            itemBuilder: (_, i) => Dismissible(
                                  ...
                                  onDismissed: (direccion) => cartBloc.add(
                                      OnDeleteProductToCartByIdEvent(
                                          listCarts[0].listCart[i].productId)),
                                  confirmDismiss:
                                          ...
                                          Container(
                                            ...
                                            child: Column(
                                              ...
                                              children: [
                                                ...
                                                CenaTextDescription(
                                                    text:
                                                        '${numberFormat.format((listCarts[0].listCart[i].price * listCarts[0].listCart[i].quantity).toInt()).toString()} vnđ',
                                                    color: CenaColors.primary)
                                              ],
                                            ),
                                          ),
                                          Expanded(
                                            child: Row(
                                              ...
                                              children: [
                                                ...
                                                CenaTextDescription(
                                                    text:
                                                        '${listCarts[0].listCart[i].quantity}',
                                                    color: CenaColors.primary),
                                                const SizedBox(width: 10.0),
                                                Container(
                                                    alignment: Alignment.center,
