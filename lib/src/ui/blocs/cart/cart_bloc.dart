...
class CartBloc extends Bloc<CartEvent, CartState> with HydratedMixin {
  ...
  Future<void> _onIncreaseProductQuantity(
    ...
    emit(state.incrementProduct(event.productId, event.storeId));
  }
  ...
  Future<void> _onDecreaseProductQuantity(
    ...
    try {
      final verify = product.firstWhere((pro) =>
          pro.productId == event.productId && pro.storeId == event.storeId);

      if (verify.quantity == 1) {
        product.remove(verify);
      } else {
        emit(state.decrementProduct(event.productId, event.storeId));
      }
    } catch (e) {
      if (kDebugMode) {
        print(e);
  ...
  Future<void> _onResetQuantity(
    ...
    try {
      var verify = product.firstWhere((pro) =>
          pro.productId == event.productId && pro.storeId == event.storeId);

      verify = verify.setQuantity(inputQuantity: event.firstcount);
      _updateCart(emit);
    } catch (e) {
      if (kDebugMode) {
        print(e);
  ...
  Future<void> _onAddProductToCart(
    ...
    try {
      ...
      final verify = product.where((pro) =>
          pro.productId == event.productCart.productId &&
  ...
  Future<void> _onIncrementQuantityProductToCard(
    ...
    product[event.plus].increament(inputQuantity: 1);
    _updateCart(emit);
  }
  ...
  Future<void> _onDecreaseQuantityProductToCart(
      ...
      Emitter<CartState> emit) async {
    product[event.subtract].decrement(inputQuantity: 1);
    _updateCart(emit);
  }
  ...
  Future<void> _onDeleteProductToCartById(
    ...
    product.removeWhere((e) => e.productId == event.productId);
    _updateCart(emit);
  }
